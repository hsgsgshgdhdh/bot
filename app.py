...
def elia5():
    ...
    """)
            print(f"{P} ▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬{J22} [𝑬𝑳𝑰𝑨] {P}▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬")
elia5()
tokn = '8874281599:AAHcxKr6ZYu8uodBafvKntY1T2lpMLuvB54'
iid = '6904264075'
os.system(' clear ')
elia5()
...
def get_rest_with_retry(username, max_retries=5):
        ...
        try:
            ...
            payload = {
                'params': json.dumps({
                    ...
                    "server_params": {
                        "device_id": "android-5b5d316bb3d314b1",
                        "waterfall_id": str(uid)
                    }
                }),
            ...
            headers = {
                'User-Agent': "Instagram 370.1.0.43.96 Android (34/14; 480dpi; 1080x2352; TECNO; TECNO LH7n)",
...
def get_user_info(username):
    ...
    is_verified = parse_between(res.text, '"is_verified":', ',')
    is_private = parse_between(res.text, '"is_private":', '},')
    if len(is_private) > 5:
        is_private = parse_between(res.text, '"is_private":', ',')
    full_name = parse_between(res.text, '"full_name":"', '"')
    followers = parse_between(res.text, '"follower_count":', ',')
    following = parse_between(res.text, '"following_count":', ',')
    pk = parse_between(res.text, '"pk":"', '"')
    bio = parse_between(res.text, '"biography":"', '"')
    
    if not pk:
        url2 = "https://www.instagram.com/api/v1/users/web_profile_info/"
        ...
        res2 = requests.get(url2, params=params, headers=headers2)
        is_verified = parse_between(res2.text, '"is_verified":', ',')
        is_private = parse_between(res2.text, '"is_private":', '},')
        if len(is_private) > 5:
            is_private = parse_between(res2.text, '"is_private":', ',')
        full_name = parse_between(res2.text, '"full_name":"', '"')
        followers = parse_between(res2.text, '"edge_followed_by":{"count":', '}')
        following = parse_between(res2.text, '"edge_follow":{"count":', '}')
        pk = parse_between(res2.text, '"id":"', '"')
        bio = parse_between(res2.text, '"biography":"', '"')
    
    year_info = get_year_from_id(pk)
    verified = 'True' if is_verified == 'true' else 'False'
    private = 'True' if is_private == 'true' else 'False'
    
    return {
        'full_name': full_name if full_name else 'Not found',
        'bio': bio[:200] if bio else 'Not found',
        'followers': followers if followers else '0',
        'following': following if following else '0',
        'private': private,
        'verified': verified,
        'pk': pk if pk else 'Unknown',
        'year': year_info
    }

def main(username):
    global tokn,iid
    rest_info = get_rest_with_retry(username)
...
"""
    print(message)
    requests.post(f'https://api.telegram.org/bot{tokn}/sendMessage?chat_id={iid}&text={message}')
    ...
    while True:
        try:
            ...
            data = {
                ...
                'variables': '{"enable_integrity_filters":true,"id":"' + str(Id) + '","__relay_internal__pv__PolarisCannesGuardianExperienceEnabledrelayprovider":true,"__relay_internal__pv__PolarisCASB976ProfileEnabledrelayprovider":false,"__relay_internal__pv__PolarisWebSchoolsEnabledrelayprovider":false,"__relay_internal__pv__PolarisRepostsConsumptionEnabledrelayprovider":false}',
                'server_timestamps': 'true',
                'doc_id': '26672929172408668',
            }

            response = session.post('https://www.instagram.com/api/graphql', headers=headers, data=data, timeout=10)
            ...
            if response.status_code == 200:
                ...
                if user and user.get('username'):
                    ...
                    if username:
                        infoinsta[username] = user
                        rr = infoinsta.get(username, {})
                        follower_count = json_data['data']['user'].get('follower_count', 0)
                        if '_' not in username and follower_count > 1:
                            email = username + "@gmail.com"
                            elia6(email)
            else:
                time.sleep(0.5)
